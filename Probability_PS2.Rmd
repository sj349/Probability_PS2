...
Another example of the prosecutor's fallacy would be to confuse P(T|D) and P(D|T) in the prior problems. The probability of getting a positive test result given that you have the disease is very different from the probability of having the disease given that you got a positive test result. This is because the prior probability of having the disease (P(D)) is so low (0.01 or 0.05 in our examples). Therefore, in one case (say, P(D|T)), we have this low prior probability in the numerator, while in the other case (P(T|D)) we have this low probability in the denominator--the fact that we multiply by the prior probability of disease in one calculation and divide by it in the other illustrates that the conditional probabilities will be drastically different. 
